...
public class PortalManager {
	...
	public boolean isOverlappingPortal(Cube c){
		for (Portal p : getPortals()){
			if (p.getArea().intersectsCube(c)) return true;
		}
		return false;
	}
	...
	public Portal getPortal(Location loc){
		for (Portal p : portals){
			if(p.getArea().containsPoint(loc)) return p;
		}
		return null;
	}
	...
	public void addPortal(Portal p){
		if(portals == null) portals = new HashSet<Portal>();
		portals.add(p);
	}
	...
	private void importData() {
		...
		} else portals = new HashSet<Portal>();
	}
