...
public class QuarryManager {
	...
	public boolean isOverlappingQuarry(Cube c){
		for (Quarry q : getQuarries()){
			if (q.getArea().intersectsCube(c)) return true;
		}
		return false;
	}
	...
	public Quarry getQuarry(Location loc){
		for (Quarry q : quarries){
			if(q.getArea().containsPoint(loc)) return q;
		}
		return null;
	}

	public void addQuarry(Quarry q){
		if(quarries == null) quarries = new HashSet<Quarry>();
		quarries.add(q);
	}
	...
	private void importData() {
		...
		} else quarries = new HashSet<Quarry>();
	}
